...
export class World {
  constructor(scene, options = {}) {
    ...
    const defaults = {
      ...
      depth: 64,
      height: 48,
      seed: Math.floor(Math.random() * 10_0000),
      terrainScale: 0.045,
      minTerrainHeight: 6,
      maxTerrainHeight: 28,
      caveFrequency: 0.09,
    ...
    this.width = Math.max(16, Math.floor(this.options.width));
    this.height = Math.max(16, Math.floor(this.options.height));
    this.depth = Math.max(16, Math.floor(this.options.depth));

    const size = this.width * this.height * this.depth;
    this.data = new Uint8Array(size);
    this.noise = new PerlinNoise(this.options.seed);
    this.mesh = null;
  ...
  generateTerrain() {
    const maxTerrainHeight = Math.min(this.height - 6, this.options.maxTerrainHeight);
    const minTerrainHeight = Math.max(4, this.options.minTerrainHeight);
    const frequency = this.options.terrainScale;
    const stoneThreshold = 5;
    const caveFrequency = this.options.caveFrequency;
    const caveThreshold = this.options.caveThreshold;
    const sandLevel = Math.floor(this.height * 0.25);
  ...
  scatterTrees() {
    ...
    const targetDensity = THREE.MathUtils.clamp(this.options.treeDensity, 0, 0.35);
    ...
    for (let x = 2; x < this.width - 2; x += 1) {
      for (let z = 2; z < this.depth - 2; z += 1) {
        ...
        if (Math.random() > targetDensity) continue;
